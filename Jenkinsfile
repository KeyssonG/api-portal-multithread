pipeline {
    ...
    environment {
        ...
        GITOPS_BRANCH = "main"
        DEPLOYMENT_FILE = "react-multithread-interno/multithread-interno.deployment.yaml"
                ...
                withCredentials([
                    ...
                    powershell script: '''
                        ...
                        git -C $gitopsDir diff --cached --quiet; if ($LASTEXITCODE -ne 0) {
                            git -C $gitopsDir -c user.name=Jenkins -c user.email=jenkins@pipeline.com commit -m "react-multithread-interno: atualiza imagem Docker para ${env:IMAGE_TAG}"
