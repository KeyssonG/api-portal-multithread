pipeline {
    ...
    stages {
        ...
        stage('Deploy no Kubernetes') {
            steps {
                powershell script: '''
                    kubectl set image deployment/multithread-interno-deployment multithread-interno-container=$env:DOCKERHUB_IMAGE:$env:IMAGE_TAG --record
                    kubectl rollout restart deployment/multithread-interno-deployment
                '''
            }
        }
    }
